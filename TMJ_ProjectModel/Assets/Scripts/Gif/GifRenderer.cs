...
namespace Gif
{
    public abstract class GifRenderer : MonoBehaviour
    {
        ...
        [SerializeField]
        List<Texture> m_ReverseTextures;
                ...
                case State.Show:
                    {
                        ...
                        if (m_Elapsed > m_Delay)
                        {
                            ...
                            // Adicione este bloco para adicionar as texturas em ordem reversa
                            if (m_Index == m_Textures.Count - 1)
                            {
                                AddReverseTextures();
                            }
                            ...
                            if (m_ReverseTextures != null && m_Index == 0)
                            {
                                m_Textures = m_ReverseTextures; // Substituir as texturas pela lista reversa
                                m_ReverseTextures = null; // Limpar a lista reversa
                            }
                        }
                    }
                    break;
        ...
        // Adicione este método para adicionar as texturas em ordem reversa
        void AddReverseTextures()
        {
            if (m_ReverseTextures == null)
            {
                m_ReverseTextures = new List<Texture>();
            }

            // Adicionar as texturas em ordem inversa ao final da lista
            for (int i = m_Textures.Count - 1; i >= 0; i--)
            {
                m_ReverseTextures.Add(m_Textures[i]);
            }
        }
